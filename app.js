...
// API URL - ЗАМЕНИ НА СВОЙ после деплоя API (БЕЗ слэша в конце!)
const API_URL = 'https://04479e0c3997.ngrok-free.app';

// Режим разработки (true = использовать демо-данные без API)
// Поставь true чтобы тестировать интерфейс без запуска API
...
async function loadUserData() {
    ...
    if (DEV_MODE) {
        throw new Error('DEV_MODE enabled');
    }
    ...
    const response = await fetch(`${API_URL}/api/user`, {
        ...
        headers: {
            ...
            'ngrok-skip-browser-warning': 'true'  // Пропускаем страницу ngrok
        }
    });
...
async function buyPlan(planId, price) {
    ...
    if (DEV_MODE) {
        showModal('⚠️', 'DEV MODE', 'API отключен в режиме разработки');
        return;
    }
